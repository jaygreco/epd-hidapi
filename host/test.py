...
# TODO: rename these more better-er
...
def write_data(dev, data):
    ...
    return __send_cmd(dev, CMD.NONE, pack_data(data))

def checksum(dev, length):
    print(f"running checksum ({length} bytes)")
    length_split = [((length >> 8*x) & 0xFF) for x in range(3)]
    __send_cmd(dev, CMD.BUF_CHECKSUM, length_split)
    res = dev.read(1, 0.1) # TODO: not working

    print(f"res: {res}")

    return res

def panel_refresh(dev):
    print("refreshing panel")
    return __send_cmd(dev, CMD.PANEL_REFRESH)

def system_restart(dev):
    pass
...
def pack_data(data):
    # split data into 63-byte chunks so it can be prefixed with CMD.BUF_SET_DATA 
    CHUNK_SIZE = 63
    data_split = chunks(data, CHUNK_SIZE)
    # insert a CMD.BUF_SET_DATA before each packet
    flattened_list = [item for inner_list in data_split for item in [CMD.BUF_SET_DATA] + inner_list]
